...
public class RouteModelHibImp implements RouteModelInt  {
	...
	public long add(RouteDTO dto) throws ApplicationException, DuplicateRecordException {
		 ...
		 RouteDTO existDto = null;
			...
			try {
				...
				throw new ApplicationException("Exception in Route Add " + e.getMessage());
			} finally {
				session.close();
			}
	...
	public void delete(RouteDTO dto) throws ApplicationException {
		...
		try {
			...
			throw new ApplicationException("Exception in Route Delete" + e.getMessage());
		} finally {
			session.close();
		}
	...
	public void update(RouteDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		/*
		 * Transaction tx = null; RouteDTO exesistDto = findByLogin(dto.getLogin());
		 * 
		...
		try {
			...
			throw new ApplicationException("Exception in Route update" + e.getMessage());
		} finally {
			session.close();
		}
	...
	public RouteDTO findByLogin(String login) throws ApplicationException {
		...
		try {
			...
			e.printStackTrace();
			throw new ApplicationException("Exception in getting Route by Login " + e.getMessage());
				...
				if (dto.getName() != null && dto.getName().length() > 0) {
				 ...
				 if (dto.getAllowedSpeed() > 0) {
				  ...
				  if (dto.getVehicleID()!= null && dto.getVehicleID().length() > 0) {
					...
					throw new ApplicationException("Exception in Route search");
				} finally {
					session.close();
				}
